...
function init() {
	...
	dom = {
		...
		show: function (el, show = true) {
			...
			for (var i = 0; i < el.length; i++) cls(dom[el[i]], "hide", show ? REM : ADD);
		},
	};
...
*/
	...
	if (window.innerWidth > 360) cls(dom.library, "unfold", ADD);
}
...
function prepAudio(id) {
	...
	a.onerror = function () {
		...
		errorcount++;
		if (errorcount < maxerrors) playNext();
		else errorcount = 0;
	};
		...
		if (i != "/") {
			...
			for (f in folder[i]) {
				if (f.toLowerCase().endsWith(".jpg") || f.toLowerCase().endsWith(".png")) {
					if (!cover || f.toLowerCase().startsWith("cover")) cover = f;
					delete folder[i][f];
				}
			}
			for (f in folder[i]) {
...
function libClick(e, context = false) {
	if (cls(e.target, "folder")) context ? addFolder(e) : openFolder(e);
	else context ? addSongNext(e) : addSong(e);
}
...
function addSong(e) {
	e.stopPropagation();
	var li = e.target;
	if (cfg.enqueue || cfg.locked) add(li.id);
	else {
		load(li.id, "next");
		if (cfg.playlist[cfg.index + 1].path != li.path)
			// Other songs are set to be played first
...
function findItem(e) {
	...
	e.preventDefault();
	if (e.target.tagName == "LI") setFilter(e.target.firstChild.textContent.trim());
	else if (e.target.tagName == "SPAN") setFilter(e.target.textContent.replace(/^\(|\)$/g, ""));
}
...
function prepDrag(e) {
	...
	if (cfg.index != -1) cls(dom.playlist.childNodes[cfg.index], "playing", REM);
...
function moveItem(drag, to, indexfrom, indexto) {
	...
	if (cfg.index != -1) {
		if (indexfrom == cfg.index) cfg.index = indexfrom < indexto ? indexto - 1 : indexto;
		else if (indexfrom < cfg.index && indexto > cfg.index) cfg.index--;
		else if (indexfrom > cfg.index && indexto <= cfg.index) cfg.index++;
	}
}
...
function removeItem(e) {
	...
	resizePlaylist();
	if (cfg.index != -1 && index <= cfg.index) cfg.index--;
	if (cfg.index != -1 && !playing) cls(dom.playlist.childNodes[cfg.index], "playing", ADD);
	endDrag();
}

function getIndex(li) {
	if (li.id == "last")
		// Temporary last item for dragging
...
//			log(nfo.groups);
			...
			if (i < 1) {
				var nfo,
					artalb = path.substring(path.lastIndexOf("/", path.lastIndexOf("/") - 1) + 1, path.lastIndexOf("/"));
				if (artalb.indexOf(" -") == -1) nfo = { artist: artalb };
				else
					nfo = {
...
function seek(e) {
	if (e == "c") {
		...
		dom.time.textContent = timeTxt(~~(dom.seek.value * audio[track].duration)) + " / " + timeTxt(~~audio[track].duration);
	}
}
...
function previous() {
	if (cfg.locked) return;
	if (cfg.index > 0) play(cfg.index - 1);
	else log("No previous item in playlist");
}
	...
	if (cfg.playlist.length > cfg.index + 1) {
		...
		if (cfg.locked || cfg.after == "randomlibrary" || cfg.after == "randomfiltered") {
			...
			do {
				...
				if (played.indexOf(next.toString()) != -1) next = null;
			...
			if (next != -1 && next < songs.length) load(songs[next].id, true);
			else {
				log("End of library. Starting at the top");
				load(songs[0].id, true);
			}
		} else if (cfg.after == "repeatplaylist" && cfg.playlist.length > 0) return load(0);
	}
}
...
function shareWhatsApp(type) {
	...
	if (share.value || type == "folder") {
		...
		if (msg != null) window.open("https://api.whatsapp.com/send?text=" + msg + " " + base + "?play=c:" + escBase64(btoa(root + share.value)));
	}
}
...
function prepPlaylists(action) {
	...
	xhttp.onload = function () {
		...
		if (playlists.length != []) {
			for (var p in playlists) playlistElements += action == "share" ? '<option value="' + p + '">' : '<button class="add">' + p + "</button>";
...
function removePlaylist(e) {
	...
	var xhttp = new XMLHttpRequest();
	xhttp.onload = function () {
		if (this.responseText != "") alert(s_error + "\n\n" + this.responseText);
		else if (cls(dom.playlists, "hide")) menu("load");
		else {
			dom.playlists.removeChild(e.target);
			if (!dom.playlists.hasChildNodes()) dom.playlists.innerHTML = '<p tabindex="1">' + s_noplaylists + "</p>";
			setFocus(dom.playlists.firstElementChild);
		}
	};
...
function savePlaylist() {
	...
	if (name) {
		name = name.replace(/[\\\/:*?"<>|]/g, " ");
		for (var pl in playlists) if (pl == name && !confirm(s_overwrite)) return;
		var xhttp = new XMLHttpRequest();
		xhttp.onload = function () {
			if (this.responseText != "") alert(s_error + "\n\n" + this.responseText);
		};
...
function fillPlaylist(pl) {
	if (pl.constructor !== Array) {
		if (confirm(s_restoreposition)) cfg.index = pl.index + cfg.playlist.length;
		pl = pl.playlist;
	}
...
function add(id, next = false) {
	...
	if (cfg.playlist.length > 0) {
		...
		for (; i < cfg.playlist.length && (next ? cfg.playlist[i].playNext : true); i++) {
			if (s.path == cfg.playlist[i].path) return setToast({ className: "error", textContent: s_alreadyadded });
		}
	}
...
function start(a) {
	...
	if (typeof promise !== "undefined")
		promise.catch(function (e) {
			log(e, true);
			if (e.code == 9) setToast({ className: "error", textContent: s_errorfile });
			else if (autoplay && e.name == "NotAllowedError") setToast({ className: "error", textContent: s_errorautoplay });
		});
	dom.seek.disabled = 0;
}
...
function toggle(e) {
	...
	switch (button.id) {
		...
		case "logbtn":
			...
			if (cls(dom.logdiv, "hide", TOG)) dom.log.blur();
			else dom.log.focus();
			return;
		case "trash":
...
function buildFilteredLibrary() {
	...
	for (var s in songs) if (songs[s].path.toLowerCase().indexOf(term) != -1) filteredsongs.push(songs[s]);
}

function toggleLock() {
	if (!password()) return;
	if (!cfg.locked && cls(dom.options, "playlistbtn")) dom.playlistbtn.click();
	cfg.locked ^= true;
...
function password() {
	...
	var pass = 0;
	for (var i = 0; i < p.length; i++) pass = pass * 7 + p.charCodeAt(i);
...
function menu(e) {
	...
	if (el.style.display == "" && e.type !== "mouseleave") {
		...
		switch (el) {
			case dom.playlists:
				if (!cls(dom.options, "playlistbtn")) dom.playlistbtn.click();
				prepPlaylists("load");
				break;
...
function filter(instant = false) {
	...
	ffor(tree, function (f) {
		...
		if (cls(f, "open")) f.className = "folder" + (cls(f, "dim") ? " dim" : "");
	});
		...
		ffor(tree, function (f) {
			...
			if (path.toLowerCase().indexOf(term) != -1) {
				...
				for (var p = f.parentNode; p && p !== dom.tree; p = p.parentNode) {
					...
					if (cls(p, "folder")) p.className = "parent folder open" + (cls(p, "filtered") ? " filtered" : "") + (cls(p, "dim") ? " dim" : "");
				}
			}
		});
	}
...
function cls(el, name, act = null) {
	...
	if (act == SET) return name == (el.className = name);
	if (!found && act >= TOG) el.classList.add(name);
	else if (found && act <= TOG) el.classList.remove(name);
...
function setFilter(f) {
	if (mode) return;
	if (typeof f === "string") dom.filter.value = f;
	else if (cfg.index == -1) return;
	else if (f.target && f.target.id == "album") {
		f = cfg.playlist[cfg.index].path.split("/");
		if (f.length < 2) return;
		dom.filter.value = f[f.length - 2];
	} else dom.filter.value = f.target.textContent;
	filter();
...
function keyNav(el, direction) {
	...
	if (dom.tree.contains(el)) {
		switch (direction) {
			case "up":
				if (el.previousElementSibling) {
					...
					while (cls(to, "open")) to = to.lastElementChild.lastElementChild;
				} else to = el.parentNode.parentNode;
				break;
			case "down":
				if (cls(el, "open")) {
					...
					while (parent.nextElementSibling == null && parent.parentNode) parent = parent.parentNode.parentNode;
					to = parent.nextElementSibling;
				}
				break;
			case "left":
				if (cls(el, "open"))
					if (cls(el, "parent")) return cls(el, "open", REM);
					else return el.click();
				else to = el.parentNode.parentNode;
				break;
			case "right":
				if (cls(el, "open") && !cls(el, "parent")) keyNav(el, "down");
				else if (cls(el, "folder")) el.click();
				return;
			case "first":
		...
		switch (direction) {
			case "up":
				if (el.previousElementSibling) return setFocus(el.previousElementSibling);
			case "right":
				return setFocus(el.parentNode.lastElementChild);
			case "down":
				if (el.nextElementSibling) return setFocus(el.nextElementSibling);
			case "left":
	...
	if (to.style.display == "none") return keyNav(to, direction);
...
function changeTheme() {
	...
	var prev = cfg.theme;
	if (cfg.theme == themes[0]) themes.push(themes.shift());
	cfg.theme = themes[0];
	cls(dom.doc, "dim", ADD);
	setTimeout(function () {
